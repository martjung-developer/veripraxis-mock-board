...
function achieveDelay(index: number): string { return `${index * 0.04}s` }
...
function StatusBadge({ status, passed }: { status: SubmissionStatus; passed: boolean | null }) {
  if (status === 'in_progress') return <span className={styles.badge} style={{ background: '#f0f9ff', color: '#0369a1', border: '1px solid #bae6fd' }}>In Progress</span>
  if (status === 'submitted')   return <span className={styles.badge} style={{ background: '#fffbeb', color: '#92400e', border: '1px solid #fde68a' }}>Submitted</span>
  if (status === 'graded')      return <span className={styles.badge} style={{ background: '#f5f3ff', color: '#5b21b6', border: '1px solid #ddd6fe' }}>Under Review</span>
...
export default function ProgressPage() {
  ...
  // ── Fetch — only runs after auth resolves ──────────────────────────────
  const fetchData = useCallback(async () => {
    if (!user) return    // guard: never fetch without a confirmed user

    setDataLoading(true)
    setDataError(null)

    try {
      const studentId = user.id   // ← always the logged-in student

      // ── 1. Verify student record exists ──────────────────────────────
      const { data: studentRow, error: stuErr } = await supabase
        .from('students')
        .select('id')
        .eq('id', studentId)       // ← own data only
        .single()

      if (stuErr || !studentRow) {
        setDataError('Student record not found.')
        setDataLoading(false)
        return
      }

      // ── 2. All submissions for this student ───────────────────────────
      //    Fetching all statuses gives a full picture of activity.
      //    Scores are only exposed for 'released' rows (enforced below).
      const { data: subRows, error: subErr } = await supabase
        .from('submissions')
        .select('id, exam_id, submitted_at, time_spent_seconds, status, percentage, passed, created_at')
        .eq('student_id', studentId)   // ← own data only
        .in('status', ['in_progress', 'submitted', 'graded', 'released'] as SubmissionStatus[])
        .order('submitted_at', { ascending: false })

      if (subErr) { setDataError('Could not load submissions.'); setDataLoading(false); return }
      ...
      if (submissions.length === 0) {
        setMetrics({
          ...
          pendingCount: 0, scoreTimeline: [], categoryAverages: [],
          recentItems: [], hasData: false,
        })
        setDataLoading(false)
        return
      }
      ...
      if (examIds.length > 0) {
        const [examRes, _] = await Promise.all([
          supabase.from('exams').select('id, title, category_id').in('id', examIds),
          Promise.resolve(null),
        ])
        exams = (examRes.data ?? []) as RawExam[]
      ...
      const streakSource    = submissions.filter(s => s.status !== 'in_progress' && (s.submitted_at ?? s.created_at))
  ...
  // Trigger fetch once auth resolves
  useEffect(() => {
    ...
    <div className={styles.page}>
      ...
      <div className={styles.header}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
          <button className={styles.filterBtn} onClick={fetchData} disabled={loading} title="Refresh data"
      ...
      {isEmpty && (
        <div className={styles.emptyState}>
          ...
          <p className={styles.emptyText}>Take your first mock exam and your progress will appear here.</p>
        </div>
      )}
      ...
      {!isEmpty && (
        <div className={styles.row2wide}>
                ...
                : (
                  <div className={styles.tableWrap}>
                    <table className={styles.table}>
                      <thead><tr><th>Exam</th><th>Score</th><th>Status</th><th>Date</th></tr></thead>
                      <tbody>
                        {(metrics?.recentItems ?? []).map(item => (
                          <tr key={item.id}>
                            ...
                            <td><StatusBadge status={item.status} passed={item.passed} /></td>
                            <td>
          ...
          <div className={styles.card}>
            <div className={styles.cardHead}><span className={styles.cardTitle}>Study Distribution</span></div>
            {loading
