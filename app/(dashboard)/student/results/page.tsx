...
// 'submitted' = pending faculty review  → shown in the pending notice, NOT in the table
// 'graded'    = internal faculty state  → never shown to students
// 'released'  = faculty explicitly released the score → shown in table
const RELEASED_STATUS = 'released' as const

// Statuses that contribute to the "awaiting review" notice
...
export default function ResultsPage() {
  ...
  // ── Stats — RELEASED only ─────────────────────────────────────────────────
  // Average score, highest score, and pass rate are computed ONLY from
  // submissions the faculty has explicitly released to the student.
  const fetchStats = useCallback(async (sid: string) => {
    const { data } = await supabase
      .from("submissions")
      .select("score, percentage, passed, time_spent_seconds")
      .eq("student_id", sid)
      .eq("status", RELEASED_STATUS);   // ← released only
  ...
  // ── Pending notice — submitted OR graded (awaiting release) ──────────────
  // These are NOT shown in the results table — only in the notice banner.
  const fetchPending = useCallback(async (sid: string) => {
    const { data } = await supabase
      .from("submissions")
      .select("id, submitted_at, status, exams ( title )")
      .eq("student_id", sid)
      .in("status", PENDING_STATUSES)   // submitted + graded = not yet released
  ...
  // The table ONLY shows submissions where status = 'released'.
  // Faculty controls visibility; students never see 'submitted' or 'graded' rows here.
  const fetchResults = useCallback(async (sid: string) => {
    ...
    let query = supabase
      ...
      .eq("student_id", sid)
      .eq("status", RELEASED_STATUS)    // ← only released submissions in the table
      .order("submitted_at", { ascending: false });
    ...
    if (error) { console.error("Results fetch error:", error.message); setLoading(false); return; }
    ...
    let shaped: ResultRow[] = (data ?? []).map((row: any) => {
      ...
      const examType     = (exam?.exam_type ?? "mock") as "mock" | "practice";
  ...
  useEffect(() => { if (studentId) { void fetchStats(studentId); void fetchPending(studentId); } }, [studentId, fetchStats, fetchPending]);
  useEffect(() => { if (studentId) void fetchResults(studentId); }, [studentId, fetchResults]);
    ...
    <div className={styles.page}>
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ fontSize: '0.77rem', color: '#b45309', margin: 0 }}>
              Results will appear here once your faculty releases them:&nbsp;
      ...
      <div className={styles.filterRow}>
        ...
        <div className={styles.searchWrap}>
          ...
          <input className={styles.searchInput} type="text" placeholder="Search exams or categories…" value={search} onChange={(e) => setSearch(e.target.value)} />
        ...
        <div className={styles.emptyState}>
          ...
          <p className={styles.emptyText}>
            {search || statusFilter !== "all" || categoryFilter !== "all" || tab !== "all"
              ...
              : "Your scores will appear here once your faculty reviews and releases your mock exam results."}
          </p>
          {!search && statusFilter === "all" && categoryFilter === "all" && tab === "all" && (
                    ...
                    <td>
                      <div className={styles.pctWrap}>
                        ...
                        <div className={styles.pctTrack}>
                          <div
                            className={`${styles.pctFill} ${getPctFillClass(row.percentage, styles)}`}
          ...
          <div className={styles.pageControls}>
            ...
            {pageNumbers.map((p, i) =>
              p === "…"
                ...
                : <button key={p} className={`${styles.pageNum} ${page === p ? styles.pageNumActive : ""}`} onClick={() => setPage(p as number)}>{p}</button>
            )}
