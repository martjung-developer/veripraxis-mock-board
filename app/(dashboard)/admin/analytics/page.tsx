...
export default function AnalyticsDashboardPage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      // All terminal submissions
      let allSubsQuery = supabase
        .from('submissions')
        .select('id, student_id, exam_id, status, time_spent_seconds')
        .in('status', ['submitted', 'graded', 'released']);

      // Released-only for score analytics
      let releasedSubsQuery = supabase
        .from('submissions')
        .select('id, student_id, exam_id, percentage, passed, time_spent_seconds')
        .eq('status', 'released');

      // Apply program filter by joining exam → program
      if (selectedProgram !== 'all') {
        // Get exam IDs for this program
        const { data: programExams } = await supabase
          .from('exams')
          .select('id')
          .eq('program_id', selectedProgram);
        const programExamIds = (programExams ?? []).map((e: { id: string }) => e.id);
        if (programExamIds.length === 0) {
          setOverview({ totalStudents: studentsRes.count ?? 0, totalExams: 0, totalAttempts: 0, releasedAttempts: 0, averageScore: 0 });
          setProgramPerf([]); setExamPerf([]); setTopStudents([]); setAtRisk([]);
          setEngagement({ totalAttempts: 0, releasedAttempts: 0, totalTimeMinutes: 0, avgTimePerAttempt: 0, avgAttemptsPerStudent: 0 });
          setLoading(false); return;
        }
        allSubsQuery    = allSubsQuery.in('exam_id', programExamIds);
        releasedSubsQuery = releasedSubsQuery.in('exam_id', programExamIds);
      }

      const [allSubsRes, releasedSubsRes] = await Promise.all([allSubsQuery, releasedSubsQuery]);
      ...
      for (const e of examRows) { examTitles[e.id] = e.title; examPrograms[e.id] = e.program_id; }
      ...
      for (const r of releasedSubs as { exam_id: string | null; percentage: number | null }[]) {
        ...
        programMap[pid].sum += r.percentage; programMap[pid].count += 1;
      }
      ...
      const programPerfData: ProgramPerf[] = programIds.map((pid) => ({
        program_id: pid, name: programNames[pid] ?? pid,
      ...
      for (const r of releasedSubs as { exam_id: string | null; percentage: number | null; passed: boolean | null }[]) {
        ...
        examBuckets[r.exam_id].sum += r.percentage; examBuckets[r.exam_id].count += 1;
        if (r.passed === true) examBuckets[r.exam_id].passed += 1;
      }
      const examPerfData: ExamPerf[] = Object.keys(examBuckets).map((eid) => ({
        exam_id: eid, title: (examTitles[eid] ?? eid).slice(0, 30),
      ...
      for (const r of releasedSubs as { student_id: string | null; percentage: number | null }[]) {
        ...
        if (r.percentage != null) { studentAgg[r.student_id].sumScore += r.percentage; studentAgg[r.student_id].count += 1; }
      ...
      const studentList = studentIds.filter((sid) => studentAgg[sid].count > 0).map((sid) => ({
        student_id: sid, full_name: profileNames[sid] ?? 'Unknown',
        average_score: parseFloat((studentAgg[sid].sumScore / studentAgg[sid].count).toFixed(1)),
        total_attempts: studentAgg[sid].attempts,
      }));
      setTopStudents([...studentList].sort((a, b) => b.average_score - a.average_score).slice(0, 8));
      setAtRisk(studentList.filter((s) => s.average_score < 50 || (s.total_attempts >= 3 && s.average_score < 65)).sort((a, b) => a.average_score - b.average_score).slice(0, 8));

      // Engagement
      const totalTime = allSubs.reduce((s: number, r: { time_spent_seconds: number | null }) => s + (r.time_spent_seconds ?? 0), 0) / 60;
      const totalAtt  = allSubs.length;
      const uniqueStu = studentIds.length;
      setEngagement({
        totalAttempts: totalAtt, releasedAttempts: releasedSubs.length,
  ...
  return (
    <motion.div className={styles.page} variants={containerVariants} initial="hidden" animate="visible">
      ...
      <motion.div className={styles.overviewGrid} variants={sectionVariants} initial="hidden" animate="visible">
        {[
          { icon: <Users size={18} color="#fff" strokeWidth={2.2} />,   bg: '#0d2540', value: overview?.totalStudents.toLocaleString() ?? '—',  label: 'Total Students' },
          { icon: <BookOpen size={18} color="#fff" strokeWidth={2.2} />, bg: '#4f5ff7', value: overview?.totalExams.toLocaleString() ?? '—',     label: 'Total Exams' },
          {
            icon: <Activity size={18} color="#fff" strokeWidth={2.2} />, bg: '#0891b2',
          ...
          { icon: <TrendingUp size={18} color="#fff" strokeWidth={2.2} />, bg: '#059669', value: overview ? `${overview.averageScore.toFixed(1)}%` : '—', label: 'Avg Score (Released)' },
        ].map((stat, i) => (
      ...
      <motion.div className={styles.chartsRow} variants={sectionVariants} initial="hidden" animate="visible">
        <motion.div className={styles.chartCard} variants={chartVariants}>
          ...
          <div className={styles.chartWrap}>
            {programPerf.length === 0 ? <EmptyChart /> : (
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={programPerf} margin={{ top: 4, right: 8, left: -20, bottom: 40 }}>
                  ...
                  <Bar dataKey="avg_score" radius={[5, 5, 0, 0]} maxBarSize={42}>
                    {programPerf.map((_, idx) => <Cell key={idx} fill={BAR_COLORS_PROGRAM[idx % BAR_COLORS_PROGRAM.length]} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            )}
            ...
            {examPerf.length === 0 ? <EmptyChart /> : (
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={examPerf} margin={{ top: 4, right: 8, left: -20, bottom: 40 }}>
                  ...
                  <Tooltip content={<CustomTooltip />} cursor={{ fill: '#f0f3f8' }} />
                  <Bar dataKey="avg_score" radius={[5, 5, 0, 0]} maxBarSize={42}>
                    {examPerf.map((_, idx) => <Cell key={idx} fill={BAR_COLORS_EXAM[idx % BAR_COLORS_EXAM.length]} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            )}
      ...
      <motion.div className={styles.bottomRow} variants={sectionVariants} initial="hidden" animate="visible">
          ...
          <div className={styles.tableCardHeader}>
            <div>
              <h2 className={styles.tableCardTitle}>
                <Award size={14} style={{ marginRight: 5, verticalAlign: 'middle', color: '#d97706' }} /> Top Students
              </h2>
              <p className={styles.tableCardSub}>Sorted by average score (released exams)</p>
            </div>
          </div>
          <div className={styles.tableBody}>
            {topStudents.length === 0 ? <EmptyTable label="No data available" /> : (
              topStudents.map((s, i) => (
                <motion.div key={s.student_id} className={styles.tableRow} custom={i} variants={tableRowVariants} initial="hidden" animate="visible">
                  ...
                  <span className={`${styles.scoreChip} ${scoreClass(s.average_score)}`}>{s.average_score.toFixed(1)}%</span>
                </motion.div>
              ))
            )}
          </div>
        ...
        <motion.div className={styles.tableCard} variants={cardVariants}>
          <div className={styles.tableCardHeader}>
            <div>
              <h2 className={styles.tableCardTitle}>
                <AlertTriangle size={14} style={{ marginRight: 5, verticalAlign: 'middle', color: '#dc2626' }} /> At-Risk Students
              </h2>
              <p className={styles.tableCardSub}>Low score or high attempts, low result</p>
            </div>
          </div>
          <div className={styles.tableBody}>
            {atRisk.length === 0 ? <EmptyTable label="No at-risk students" /> : (
              atRisk.map((s, i) => (
                <motion.div key={s.student_id} className={styles.tableRow} custom={i} variants={tableRowVariants} initial="hidden" animate="visible">
                  <div className={styles.studentAvatar}>{initials(s.full_name)}</div>
                  <div className={styles.studentInfo}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
                    <span className={`${styles.scoreChip} ${scoreClass(s.average_score)}`}>{s.average_score.toFixed(1)}%</span>
                    <span className={styles.riskBadge}><span className={styles.riskDot} /> At-Risk</span>
                  </div>
                </motion.div>
              ))
            )}
        ...
        <motion.div className={styles.engagementCard} variants={cardVariants}>
          <div className={styles.engagementHeader}>
            <h2 className={styles.engagementTitle}>
              <Clock size={14} style={{ marginRight: 5, verticalAlign: 'middle', color: '#0891b2' }} /> Engagement
            </h2>
            <p className={styles.engagementSub}>Attempt & time metrics (all submissions)</p>
          </div>
          <div className={styles.engagementBody}>
            {[
              { icon: <Activity size={15} color="#fff" />, bg: '#0891b2', label: 'Total Attempts',           sub: 'All submitted exams',           value: engagement?.totalAttempts.toLocaleString() ?? '—',     unit: 'attempts' },
              { icon: <Award size={15} color="#fff" />,    bg: '#059669', label: 'Released Attempts',        sub: 'Scores visible to students',    value: engagement?.releasedAttempts.toLocaleString() ?? '—',  unit: 'released' },
              {
                icon: <Clock size={15} color="#fff" />,  bg: '#7c3aed', label: 'Total Time Spent', sub: 'Cumulative across all students',
                value: engagement ? (engagement.totalTimeMinutes >= 60 ? `${(engagement.totalTimeMinutes / 60).toFixed(1)}h` : `${engagement.totalTimeMinutes}m`) : '—',
                unit: 'total time',
              },
              { icon: <Users size={15} color="#fff" />,    bg: '#d97706', label: 'Avg Attempts / Student',   sub: 'Engagement depth',              value: engagement?.avgAttemptsPerStudent.toString() ?? '—',   unit: 'per student' },
            ].map((m, i) => (
              <div key={i} className={styles.engagementMetric}>
                <div className={styles.engagementMetricLeft}>
                  <div className={styles.engagementMetricIcon} style={{ background: m.bg }}>{m.icon}</div>
                  <div>
