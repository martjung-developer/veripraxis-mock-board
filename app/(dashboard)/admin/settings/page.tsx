...
function getPasswordStrength(pw: string): 'weak' | 'fair' | 'strong' | null {
  ...
  const score = [hasUpper, hasLower, hasNumber, hasSpecial, pw.length >= 12].filter(Boolean).length
...
export default function SettingsPage() {
  ...
  const handleAvatarChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_BYTES) {
      setToast({ type: 'error', message: 'Image must be smaller than 2 MB.' })
      return
    }
    ...
    if (storageError) {
      setUploading(false)
      setUploadPct(0)
      setToast({ type: 'error', message: 'Avatar upload failed. Please try again.' })
      return
    }
    ...
    const { error: updateError } = await supabase
      .from('profiles')
      .update({ avatar_url: publicUrl, updated_at: new Date().toISOString() } as unknown)
      .eq('id', profile.id)
    ...
    if (updateError) {
      setToast({ type: 'error', message: 'Failed to save avatar URL.' })
    } else {
    ...
    return (
      <div className={s.page}>
        <div className={s.header}>
          <div>
            <div className={s.pageTitle} style={{ color: 'var(--text-muted)' }}>Settings</div>
          </div>
        </div>
        <div className={s.layout}>
          ...
          <div className={s.content}>
            {[1,2,3].map(i => (
              <div key={i} className={s.card} style={{ height: 160, background: 'var(--surface-tint)' }} />
            ))}
          </div>
        </div>
      </div>
        ...
        <div className={s.content}>
                ...
                <div className={s.avatarUploadInfo}>
                  ...
                  {uploading && uploadPct > 0 && (
                    <div className={s.uploadProgress}>
                      <div className={s.uploadProgressFill} style={{ width: `${uploadPct}%` }} />
                    </div>
                  )}
                </div>
              </div>
              ...
              >
                <div style={{ maxWidth: 420, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                      ...
                      <>
                        <div className={s.pwStrength}>
                          <div className={`${s.pwStrengthFill} ${
                            pwStrength === 'weak'   ? s.pwStrengthWeak   :
                            pwStrength === 'fair'   ? s.pwStrengthFair   :
                            s.pwStrengthStrong
                          }`} />
                        </div>
                        <span className={`${s.pwStrengthLabel} ${
                          pwStrength === 'weak'   ? s.pwStrengthLabelWeak   :
                          pwStrength === 'fair'   ? s.pwStrengthLabelFair   :
                          s.pwStrengthLabelStrong
                        }`}>
                  ...
                  <motion.div className={s.fieldGroup} variants={fieldVariants} style={{ marginBottom: 0 }}>
                    ...
                    <div style={{ position: 'relative' }}>
                      <input
                        className={`${s.input} ${confirmPw && confirmPw !== newPw ? s.inputError : ''}`}
          ...
          {activeSection === 'preferences' && (
            ...
            >
              ...
              <div className={s.toggleRow}>
                <div className={s.toggleLeft}>
                  <p className={s.toggleTitle}>
                    <Moon size={13} style={{ marginRight: 6, verticalAlign: 'middle', opacity: 0.6 }} />
                    Dark Mode
                  </p>
                  <p className={s.toggleDesc}>Switch the interface to a dark colour scheme.</p>
                </div>
                <button
                  className={`${s.toggle} ${darkMode ? s.toggleOn : ''}`}
              ...
              <div className={s.toggleRow}>
                <div className={s.toggleLeft}>
                  <p className={s.toggleTitle}>
                    <Bell size={13} style={{ marginRight: 6, verticalAlign: 'middle', opacity: 0.6 }} />
                    Push Notifications
                  </p>
                  <p className={s.toggleDesc}>Receive in-app alerts for submissions and deadlines.</p>
                </div>
                <button
                  className={`${s.toggle} ${notifEnabled ? s.toggleOn : ''}`}
              ...
              <div className={s.toggleRow}>
                <div className={s.toggleLeft}>
                  ...
                  <p className={s.toggleDesc}>Get email updates when students submit exams.</p>
                </div>
                <button
                  className={`${s.toggle} ${emailNotif ? s.toggleOn : ''}`}
          ...
          {activeSection === 'danger' && (
            ...
            >
              ...
              {/* Delete account — shown as disabled / intent only */}
              <div className={s.dangerItem}>
                <div className={s.dangerItemInfo}>
                  <p className={s.dangerItemTitle}>Delete account</p>
                  <p className={s.dangerItemDesc}>
                    Permanently remove your account and all associated data. This action cannot be undone.
                    Contact a system administrator to proceed.
                  </p>
                </div>
                <motion.button
                  className={s.buttonDanger}
